...
regularization = 0.4 #正则化系数lambda
...
def residuals_func(p, y, x):
    ret = fit_func(p, x) - y
    ret = np.append(ret, np.sqrt(regularization) * p) #将lambda^(1/2)
    return ret


x = np.linspace(0, 1, 9) #随机选择9个点作为x
x_points = np.linspace(0, 1, 1000) #画图时需要连续的点
...
print('Fitting Paraments', plsq[0]) #输出拟合参数
